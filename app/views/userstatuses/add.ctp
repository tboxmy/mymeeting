...
<?php echo $form->create('Userstatus',array('type'=>'file','url'=>array('committee'=>$dcommittee['Committee']['short_name'],'decisionid'=>$this->params['decisionid'])));?>
    <fieldset>
        <legend><?php __('Add User Status');?></legend>
<?php

echo $form->hidden('decision_id',array('value'=>$this->params['decisionid']));
echo $form->hidden('user_id',array('value'=>$this->params['group_id']));
if(isset($oldstatus)){
    echo $form->input('description',array('value'=>$oldstatus['Userstatus']['description']));
}
else{
