<?php
/* SVN FILE: $Id: form.php 7639 2008-09-21 04:06:37Z nate $ */
/**
 ...
 * @lastmodified	$Date: 2008-09-21 12:06:37 +0800 (Sun, 21 Sep 2008) $
 ...
 */
	function select($fieldName, $options = array(), $selected = null, $attributes = array(), $showEmpty = '') {
        ...
        $jsninja=null;
        foreach($attributes as $atname=>$atdat){
            if(strtolower(substr($atname,0,2))=='on'){
                $jsninja[$atname]=$atdat;
            }
        }
		...
		$select = array_merge($select, $this->__selectOptions(array_reverse($options, true), $selected, array(), $showParents, array('escape' => $escapeOptions, 'style' => $style,'jsninja'=> $jsninja)));
			...
			if ($name !== null) {
				...
				if ($showParents || (!in_array($title, $parents))) {
					...
					if ($attributes['style'] === 'checkbox') {
                        ...
                        if(isset($attributes['jsninja'])){
                            $htmlOptions=array_merge($htmlOptions,$attributes['jsninja']);
                        }
