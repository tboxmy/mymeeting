...
<table cellpadding='0' cellspacing='0' border='0' id="statuses">
    ...
    ?>
    <tr<?php echo $class?>>
        <td><?php echo $status['Group']['name']?></td>
        <td><?php echo nl2br($status['Groupstatus']['description'])?><br/>
        <?php
        ...
        <td><?php echo date(Configure::read('date_format'),strtotime($status['Groupstatus']['action_date']));?></td>
        <td><?php echo $comment->list_comment('Groupstatus',$status);?></td>
    </tr>
    <?php }?>
</table>
<p>&nbsp;</p>
